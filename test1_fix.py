...
class AuthenticationSystem:
    ...
    def login(self, username, password):
        ...
        user = User(user_row['user_id'].values[0], user_row['username'].values[0], user_row['password'].values[0], 3 - user_row['failed_attempts_left'].values[0], user_row['is_locked'].values[0])

        if user.is_locked:
            print(f"Account for {username} is locked. Please contact support.")
            return

        if not password == user_row['password'].values[0]:
